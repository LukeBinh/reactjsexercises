...
const Ex4 = () => {
    ...
    const isCheck = (e) => {
        if(e.target.checked === true){
            ...
            return setIsCheckedList(isCheckedList.filter((item, index) => {if(index !== itemIndex){ return item} } ))
    }
  ...
  return (
    <div className='ex4 container'>
        ...
        <p>Items checked are: {isCheckedList.map((item,index) => (<span key={index}>{item} </span>))} </p>
    </div>
  )
}
